...
const Hero = ({ pages }) => {
    ...
    const handleLogin = (e) => { //boss' code
        ...
        let type = isChecked ? "pharma" : "user"
    ...
    const handleSuccessfulLogin = (data, username) => {
        ...
        if (data.authkey) {
            Cookies.set("authkey", data.authkey, { expires: 7 });
        }
        //Cookies.set("username", )

        gsap.to("#menu", { scale: 0.95, opacity: 0, duration: 1, ease: "power2.inOut" });
        if (pillRef.current && typeof pillRef.current.triggerSeparation === 'function') {
            pillRef.current.triggerSeparation("seperate");
        }

        Cookies.set("type", isChecked ? "admin" : "user", { expires: 7 });
    ...
    const handleSignup = (e) => {
        ...
        if(username == "" || password == ""){
            setLoginError('Field(s) are empty.');
            return
        }
        ...
        request
            .post(`/${type}/signup`, { username, password })
            .then(data => toggleAuthMode(data))
            .catch(err => {
                    ...
                    <div id="graphic" className="shrink-0 flex-grow h-[calc(100vh-20px)] w-full box-border relative">
                        ...
                        >
                            <path
                                ...
                                style={{ filter: 'drop-shadow(0 0 4px #8ffff8ff)' }}
                            />
                        </svg>
