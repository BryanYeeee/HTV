...
const ClientPort = () => {
  ...
  const [drugData, setDrugData] = useState([
    ...
    { name: 'A', remain: 120, schedules: ['1_1400', '3_1400', '5_1400'], model: ['rgba(114, 161, 208, 1)'] },
    ...
    <div className='bg flex justify-around items-center h-screen p-10 gap-12'>
      ...
      <div className='w-1/3 h-full flex flex-col'>
        ...
        <Switcher activeIndex={rightIndex} axis='y' className='size-full'>
          {[
            <section className='h-full border-x-8 p-8 flex flex-col items-center rounded-xl border-white'>
              <div className=' h-40 w-40 border-y-8 rounded-xl border-white'>
                <div className="relative w-full h-full">
                  <PillRender
                    topColor={curDrug.model[0]}
                    bottomColor={curDrug.model[0]} 
                    radius={0.5}
                    curDrug={curDrug}
                  />
                </div>
              ...
              <div className="border-x-8 rounded-xl border-white">{curDrug.description}</div>
            </section>,
